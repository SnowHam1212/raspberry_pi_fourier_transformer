...
1. `N=2048`個のADC値を、可能な限り高速に連続読み取り（`read_adc`をN回呼ぶだけ）
...
- 1ブロックの所要時間: `2048 / 39271 ≈ 52ms` → だいたい50msごとにスペクトラムが更新される
- 周波数分解能（1本あたりの幅）: `rate / N ≈ 19Hz`

つまり「連続的に滑らかに追従する」のではなく、「約50msごとにまとめて撮り直したスナップショットを次々に表示することで、疑似的にリアルタイムに見せている」のが実態です。
