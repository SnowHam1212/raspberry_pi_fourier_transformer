...
3連続・4連続で押すと、直近のスペクトラムで強い順の上位3つ・4つのピーク（`top_n_peaks`）を0.3秒ずつ順番にループ再生し続け（`play_topn_loop`）、もう一度スイッチを押すと止まります。
...
- スイッチを素早く3連続・4連続で押すと、上位3つ・4つのピーク周波数を0.3秒ずつループ再生し、もう一度押すと止まる
- `Esc`キーかウィンドウを閉じると終了
